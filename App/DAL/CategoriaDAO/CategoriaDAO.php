...
namespace App\DAL\CategoriaDAO;
...
class CategoriaDAO {
    ...
    */
    ...
    public function RetornarTodos() {
        try {
            $sql = "SELECT categoria_id, nome, thumb FROM categoria WHERE status = 1"; //Categorias
            ...
            foreach ($dt as $cat) {
                $categoria = new Categoria();
                $categoria->setCategoriaId($cat["categoria_id"]);
                $categoria->setNome($cat["nome"]);
    ...
    public function RetornarTodosJSON() {
        try {
            $sql = "SELECT categoria_id, nome FROM categoria WHERE status = 1 ORDER BY nome ASC"; //Categorias PAI e FILHO
