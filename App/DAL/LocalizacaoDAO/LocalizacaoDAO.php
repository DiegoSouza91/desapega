...
namespace App\DAL\LocalizacaoDAO;
...
class LocalizacaoDAO {
    ...
    */
    ...
    public function RetornarTodosJSON() {
        try {
            $sql = "SELECT id, codigoUf, Nome, Uf FROM estado WHERE status = 1"; //Categorias PAI e FILHO
            ...
            foreach ($dt as $loc) {
                ...
                $Localizacao->setEstadoId($loc["id"]);
                $Localizacao->setCodigoUf($loc["codigoUf"]);
                $Localizacao->setNome($loc["Nome"]);
                $Localizacao->setUf($loc["Uf"]);
                $listaLocalizacao[] = $Localizacao;
            }

            return $this->serialization->serialize($listaLocalizacao);
        } catch (PDOException $ex) {
            if ($this->debug) {
                echo "ERRO: {$ex->getMessage()} LINE: {$ex->getLine()}";
            }
    ...
    public function RetornarTodosMunicipiosJSON() {
        try {
            $sql = "SELECT municipio_id, codigo, nome, uf FROM municipio WHERE status = 1 ORDER BY nome ASC"; //Categorias PAI e FILHO
            ...
            foreach ($dt as $loc) {
                ...
                $municipio->setMunicipioId($loc["municipio_id"]);
                $municipio->setCodigo($loc["codigo"]);
                $municipio->setNome($loc["nome"]);
                $municipio->setUf($loc["uf"]);
                $listaLocalizacao[] = $municipio;
            }
